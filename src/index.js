...
const tools = {
	...
	deepClone(obj) {
		const _tarType = tools.getType(obj)
  ...
  /**
    ...
    /**
		...
		const _date = tools.isEmpty(date) ? new Date() : ((date instanceof Date || tools.getType(date) == 'string' || tools.getType(date) == 'number') ? new Date(date) : new Date())
